...
export default function ChatInterfacePage() {
  ...
  const generateResponse = (): string => {
    const contextSnippet = transcript
      .replace(/\s+/g, " ")
      .trim()
      .slice(0, 140);
    const responses = [
      "Based on the story, this appears to be about a significant family memory from an earlier time. The narrator shared vivid details about their experiences and the people who were important to them.",
      "From what was mentioned in the recording, this relates to family traditions and the values that were passed down through generations. The story emphasizes the importance of these connections.",
      "The story mentions specific details about daily life and routines from that time period. These memories highlight how family relationships were built through consistent, meaningful interactions.",
      `According to the narrative, this was a formative experience that shaped the family's identity. ${contextSnippet ? `For example: "${contextSnippet}..."` : "The speaker included details about places, people, and moments that held special significance."}`,
    ];

    const i = Math.floor(Math.random() * responses.length);
    return responses[i] ?? "I'm not sure how to respond to that.";
  };
          ...
          <Card className="p-4 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200">
            <div className="flex items-center gap-2">
              ...
              <p className="text-xs text-amber-800">
                Ask questions to explore details, understand context, or find specific information from this family story
              </p>
            </div>
          </Card>
        </motion.div>
